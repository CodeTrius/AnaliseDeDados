...
INTERCEPTO = 20 # Valor estimado a partir dos dados do modelo anterior
...
# Variável de Entrada: Irradiação GHI
# O GHI é o recurso solar que se correlaciona com a produção.
X = df[['GHI_Irradiacao_Wh_m2_dia']]

# Variável de Saída: Energia Produzida (o que queremos prever)
y = df['Energia_kWh']
...
# Um valor negativo alto indica baixa eficiência.
...
#print(melhores_dias[['Data_Diaria', 'Anomalia_Eficiencia_kWh','GHI_Irradiacao_Wh_m2_dia', 'Temperatura_Media_C','Velocidade_Vento_Media_ms']].sort_values(by='Anomalia_Eficiencia_kWh', ascending=False).to_string(index=False))
